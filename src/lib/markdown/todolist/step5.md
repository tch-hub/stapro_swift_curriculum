...
- **`@Binding var text: String`**
  ...
  `@Binding`自体はデータベースへ保存するものではありません。入力された値を親ビューの変数と共有し、親側で必要に応じて保存処理を行えるようにします。
...
1. **データ型の定義**  
   ...
   複数の商品データを配列で用意してください。

2. **`SearchBar` の作成**  
   以下のプロパティを持つ構造体を定義してください。
   - `@Binding var searchText: String`（検索テキストを親と共有）
   - `var placeholder: String = "商品を検索..."`（デフォルト値あり）

3. **入力チェック**  
   `private var hasText: Bool` プロパティで、検索テキストが入力されているかを判定してください。

4. **UIの実装**  
   `HStack` を使って以下を横並びに配置してください。
   - `TextField`（`magnifyingglass` アイコン付き、`Capsule` 背景）
   - クリアボタン（`xmark.circle.fill` アイコン、`hasText` で表示切り替え）

5. **フィルタリング機能**  
   `searchText` に基づいて、商品名またはカテゴリで絞り込まれたリストを表示してください。  
   `filter { product in ... }` を使用してください。

6. **プレビューで確認**  
   `#Preview` で複数の商品を表示し、検索で絞り込めることを確認してください。
...
struct ContentView: View {
    ...
    let products = [
        Product(name: "りんご", category: "フルーツ"),
        Product(name: "にんじん", category: "野菜"),
        Product(name: "バナナ", category: "フルーツ"),
        Product(name: "キャベツ", category: "野菜"),
        Product(name: "みかん", category: "フルーツ")
    ]
    ...
    var filteredProducts: [Product] {
        ...
        return products.filter { product in
            product.name.contains(searchText) || product.category.contains(searchText)
        }
    }
