...
ここまでの実装（ステップ15〜16）で、アプリの「起動から初期データ作成、メイン画面への切り替え」という一連の流れが完成しました。

プレビューではなく、シミュレータや実機でアプリを「ビルド（実行）」して、1秒間のローディング表示を確認してみましょう。

### ビルドと実行の方法
1. Xcodeの画面左上にある「再生ボタン（▶️）」をクリックするか、キーボードの **`Command + R`** を押します。
2. シミュレータが立ち上がり、アプリが起動します。
3. 「アプリを準備中...」という画面が1秒ほど表示された後、メイン画面（MainStack）に切り替われば成功です！
