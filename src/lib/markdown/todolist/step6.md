...
2. **サンプルデータの作成**  
   `Task` 構造体（`id: UUID`、`title: String`、`isCompleted: Bool`）を定義し、複数のサンプルタスクを用意してください。

3. **Computed Propertyの実装**  
   選択中のフィルターに基づいて、タスクを絞り込む `filteredTasks: [Task]` を Computed Property で実装してください。  
   例：`pending` が選択されていれば、`isCompleted == false` のタスクのみを返す。

4. **`Menu` + `Picker` の実装**  
   タップするとフィルター方法が選べるドロップダウンメニューを作ってください。  
   ラベルには現在のフィルターモード名と `funnel.fill` アイコンを表示してください。

5. **フィルタリング表示**  
   `filteredTasks` の結果を `List` で表示し、フィルター選択で内容が変わることを確認してください。  
   タスクの完了状態も `.strikethrough()` などで視覚的に区別してください。

6. **プレビューで確認**  
   複数のフィルターを選択したときに、表示されるタスクが正しく変わることを確認してください。
...
struct ContentView: View {
    enum FilterMode: String, Identifiable, Hashable {
        case all = "すべて"
        case completed = "完了"
        case pending = "未完了"
    ...
    @State var selectedFilter: FilterMode = .all
    @State var tasks: [Task] = [
        Task(title: "買い物リストを確認", isCompleted: false),
        Task(title: "会議の資料を準備", isCompleted: true),
        Task(title: "メールを返信", isCompleted: false),
        Task(title: "コードレビュー", isCompleted: true)
    ]
            ...
            HStack {
                Menu {
                    Picker("フィルター", selection: $selectedFilter) {
                        ForEach([FilterMode.all, .completed, .pending]) { mode in
                            Text(mode.rawValue)
                                .tag(mode)
                        }
                    }
