---
title: ステップ1: アプリ制作の作成とセットアップ
summary: Xcodeでアプリ制作を作成し、不要なファイルを削除して必要なディレクトリ構成と初期ファイルを用意します。
---

### 1. Xcodeで新規アプリ制作を作成

1. Xcodeを起動し、「Create a new Xcode appCreation」を選択する。
...
新しいアプリ制作を作ると自動生成されるファイルはすべて削除し、必要なファイルを自分で作り直します。以下の手順で進めてください！

#### ファイルを削除する方法

- Xcodeのアプリ制作ナビゲーターで、削除したいファイルまたはフォルダを見つけて選択します。
...
- アプリ制作ナビゲーターの空白の部分で右クリックし、「New Folder」を選択

Xcodeで新しいファイルを作る手順（いずれかの方法でファイルを作成してください）

- Xcode メニューバーの「File」→「New」→「EmptyFile」を選択
- アプリ制作ナビゲーターの空白の部分で右クリックし、「New EmptyFile」を選択
