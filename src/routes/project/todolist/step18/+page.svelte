...
<div class="container mx-auto px-4 py-8">
	...
	<div class="mb-8 flex justify-between">
		<a href="{base}/project/todolist/step17" class="btn btn-outline">← 前のステップ</a>
		<a href="{base}/project/todolist/step19" class="btn btn-primary">次のステップ →</a>
	</div>
	...
	<div class="space-y-8">
		<div>
			...
			<p class="mb-4">
				`Screens/Views/Main`フォルダの中に、`TabManageView.swift`という名前で新しいSwiftUI
				Viewファイルを作成します。
			</p>
		</div>
...
`}
		...
		<div>
			...
			<ul class="mb-4 list-inside list-disc space-y-2 rounded-md bg-base-200 p-4">
				<li>
					<strong>`@Query(...) private var toDoTabs: [ToDoTab]`</strong>:
					`HomeView`と同じように、このビューもSwiftDataに保存されている全ての`ToDoTab`データを取得します。
				</li>
				<li>
					<strong>`List &#123; ForEach(...) &#125;`</strong>:
					取得した`toDoTabs`のリストを元に、各タブの名前を`Text`で表示します。
				</li>
				<li>
					<strong>`.toolbar &#123; ... &#125;`</strong>:
					ナビゲーションバーにボタンなどのアイテムを追加するためのモディファイアです。ここでは右端(`navigationBarTrailing`)に「＋」アイコンの`Button`を配置しています。
				</li>
			</ul>
			<p>
