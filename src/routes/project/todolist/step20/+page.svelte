...
<div class="container mx-auto px-4 py-8">
	<h1 class="mb-4 text-3xl font-bold">ステップ20: おもてなし機能：アプリ初回起動時にサンプルデータを登録する</h1>
...
`}
		...
		<div>
			...
			<ul class="mb-4 list-inside list-disc space-y-2 bg-base-200 p-4 rounded-md">
				<li><strong>`onAppear(perform: setupInitialData)`:</strong> `InitialView`が画面に表示された瞬間に`setupInitialData`メソッドを一度だけ呼び出します。</li>
				<li><strong>`DispatchQueue.main.asyncAfter(...)`:</strong> 指定した時間（ここでは1秒後）に処理を遅らせて実行します。これにより、ユーザーは「データを準備しています...」のメッセージを視認できます。</li>
				<li><strong>`todoTabService.getAll()`:</strong> まずデータベースにデータが既にあるかを確認します。</li>
				<li><strong>`if existingTabs.isEmpty { ... }`:</strong> データが空（初回起動）の場合のみ、`if`文の中のサンプルデータ登録処理が実行されます。</li>
				<li><strong>`showMainStack = true`:</strong> この変数を`true`にすることで、`body`の`if`文が再評価され、画面が`MainStack()`に切り替わります。</li>
			</ul>
		</div>
	</div>
