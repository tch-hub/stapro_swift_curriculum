...
`}
		...
		<div>
			...
			<ul class="mb-4 list-inside list-disc space-y-2 rounded-md bg-base-200 p-4">
				<li>
					<strong>`NavigationLink(value: "tabManage")`</strong>:
					これが画面遷移の「スイッチ」です。タップされると、`value`に指定された値（ここでは文字列`"tabManage"`）を`NavigationStack`に通知します。
				</li>
				<li>
					<strong>`.navigationDestination(for: String.self)`</strong>:
					`NavigationStack`は`NavigationLink`から`String`型の値を受け取ると、この処理ブロックを実行します。`destination`に`"tabManage"`が入ってくるので、if文で判定して`TabManageView`を表示する、という仕組みです。
				</li>
			</ul>
			<p>
