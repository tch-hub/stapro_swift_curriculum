...
<div class="container mx-auto px-4 py-8">
	<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
		<div class="flex flex-wrap gap-3">
			{#if data.stepId === 'step1'}
				<a href="{base}/project/timer" class="btn btn-outline btn-sm">← プロジェクト概要</a>
			{/if}
			{#if data.prevStep}
				<a href="{base}/project/timer/{data.prevStep.id}" class="btn btn-outline btn-sm">
					← {data.prevStep.title}
				</a>
			{/if}
		</div>
		{#if data.nextStep}
			<a href="{base}/project/timer/{data.nextStep.id}" class="btn btn-sm btn-primary">
				{data.nextStep.title} →
			</a>
		{/if}
	</div>
	...
	<div class="mt-10 flex flex-wrap items-center justify-center gap-4">
		...
		{/if}
		{#if data.stepId === 'step1'}
			<a href="{base}/project/timer" class="btn btn-sm btn-secondary">ステップ一覧に戻る</a>
		{/if}
		{#if data.nextStep}
			<a href="{base}/project/timer/{data.nextStep.id}" class="btn btn-sm btn-primary">
				{data.nextStep.title} →
